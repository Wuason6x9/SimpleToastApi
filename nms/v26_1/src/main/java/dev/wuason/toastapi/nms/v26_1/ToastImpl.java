package dev.wuason.toastapi.nms.v26_1;
...
public class ToastImpl implements IToastWrapper {
    ...
    public void sendToast(ItemStack icon, Player player, String title, EToastType toastType, String namespace, String path) {
        ...
        net.minecraft.world.item.ItemStack nmsStack = CraftItemStack.asNMSCopy(new ItemStack(Material.STICK));
        DataComponentPatch dataComponentPatch = DataComponentPatch.builder()
                .set(DataComponents.ITEM_MODEL, Identifier.fromNamespaceAndPath("minecraft", "air"))
                .build();
        nmsStack.applyComponents(dataComponentPatch);

        if (icon != null) {
            nmsStack = CraftItemStack.asNMSCopy(icon);
        }
